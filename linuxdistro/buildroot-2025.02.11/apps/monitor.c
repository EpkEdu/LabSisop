...
void dump_file_to_html(const char *path) {
    ...
    if (fp) {
        ...
        while (fgets(line, sizeof(line), fp)) {
            // Prevenção básica de quebra de layout HTML (substituindo < e > se existirem)
...
int main() {
    ...
    long u1, n1, s1, i1, iw1, ir1, sir1;
    long u2, n2, s2, i2, iw2, ir2, sir2;
    FILE *stat_fp = fopen("/proc/stat", "r");
    if (stat_fp) {
        fscanf(stat_fp, "cpu %ld %ld %ld %ld %ld %ld %ld", &u1, &n1, &s1, &i1, &iw1, &ir1, &sir1);
        fclose(stat_fp);
    }
    sleep(1); // Aguarda 1 segundo para medir a diferença de uso
    ...
    if (uptime_fp) {
        ...
        if (fscanf(uptime_fp, "%lf %lf", &uptime_sec, &idle_sec) == 2) {
            ...
            // Nota: Em sistemas multi-core, o idle_sec é a soma do tempo ocioso de todos os núcleos
    ...
    int cores = 0;
    char cpu_model[13] = "Desconhecido";
    char cpu_speed[13] = "Desconhecida";
    ...
    if (sysinfo(&s_info) == 0) {
        ...
        // sysinfo retorna loads em frações de 1<<SI_LOAD_SHIFT (geralmente 1<<16)
    ...
    // Operações de I/O (Leitura e Escrita em blocos)
    ...
    if (disk_fp) {
        ...
        while (fgets(line, sizeof(line), disk_fp)) {
            ...
            // No diskstats, os campos 4 e 8 após o nome do device representam as operações (reads e writes concluídos)
            if (sscanf(line, "%d %d %31s %llu %*d %*d %*d %llu", &major, &minor, dev_name, &r_io, &w_io) == 5) {
                total_reads += r_io;
                total_writes += w_io;
            }
        }
    ...
    // // Sistemas de Arquivos, Dispositivos e Rede (Dump direto do /proc)
